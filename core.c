...
ssize_t read_memory(pid_t pid, uint64_t address, void *value, size_t size) {
...
int main(int argc, const char *argv[]) {
	...
	while (fgets(line, sizeof(line), fp)) {
		if (sscanf(line, "%lx-%lx %c", &start, &end, &permission) == 3) {
			...
			if (strstr(line, "[anon:libc_malloc]") == NULL && strstr(line, "[stack]") == NULL) continue;
			...
			if (read_memory(pid, start, data, size) == -1) {
